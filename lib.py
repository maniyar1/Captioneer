...
from autosub.autosub.segmentAudio import silenceRemoval
from autosub.autosub.audioProcessing import extract_audio, convert_samplerate
from autosub.autosub.writeToFile import write_to_file

# Line count for SRT file
line_count = 0

class Subtitle:
    def __init__(self, start_time, end_time, text, index, cues):
...
def ds_process_audio(ds, audio_file, file_handle, vtt):  
    ...
    if len(infered_text) != 0:
        line_count += 1
        return Subtitle(start_time = limits[0], endTime = limits[1], index = line_count, cues = cues)
    else:
        return None
...
def run():
    ...
    # TODO: Swap *all* of this out for stuff submitted via gui
    ...
    shutil.rmtree(audio_directory) # Clear it
    os.mkdir(audio_directory) # Remake it
    ...
    for file in tqdm(sort_alphanumeric(os.listdir(audio_directory))):
        ...
        if audio_segment_path.split(os.sep)[-1] != audio_file_name.split(os.sep)[-1]:
            one_subtitle = ds_process_audio(ds, audio_segment_path, file_handle, args.vtt)
            if (one_subtitle is not None):
                print(one_subtitle)
                # TODO: Send the object over to JS via window.evaluate_js here
